...
from src.formatData import format_form_data

app = Flask(__name__)

# use lines below to connect to local MongoDB
app.config['MONGO_DBNAME'] = 'habarci_db'
app.config['MONGO_URI'] = 'mongodb://localhost:27017/habarci_db'
...
def update_jobs_deliverer_and_status__by_oId(job_id, new_status):
    ...
    if new_status == 'pending':
        ...
        result = jobs.find_one_and_update({'_id': job_id}, {
            '$set': {'status': new_status, 'delivererID': auth0ID, 'delivererName': delivererName,
        ...
        msg_body = templates.get_templates(template_type='order-completed').replace('##name##', user_name).replace(
            '##pickUpAddress##', pickUpAddress).replace('##destinationAddress##', destinationAddress)
        gmail_main.gmailv1(client=GMAIL, mode='send', email_list=[user_email],
                           header="(Habarci) YOUR REQUESTED JOB HAS BEEN COMPLETED", msg_body=msg_body)
